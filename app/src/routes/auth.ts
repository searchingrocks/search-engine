...
import { getSetting } from '../models/settings'; // New import
...
// Registration GET route with layered setting for allowRegistrations
authRouter.get('/register', async (req, res) => {
  const envAllowRegistrations = process.env.ALLOW_REGISTRATIONS;
  let allowRegistrations: boolean;
  if (envAllowRegistrations !== undefined) {
    allowRegistrations = envAllowRegistrations === 'true';
  } else {
    const setting = await getSetting('allowRegistrations');
    allowRegistrations = setting === 'true';
  }
...
// Registration POST route with layered setting for allowRegistrations
authRouter.post('/register', async (req, res) => {
  ...
  // If no users exist, make the first user admin.
